...
<div class="relative w-full h-full">
  <div class="flex items-center bg-blue-500 text-white rounded w-full hover:bg-blue-600 h-full">
    ...
    <div role="button" tabindex="0" class="w-1/5 flex items-center justify-center bg-blue-700 rounded-r cursor-pointer h-full" on:click={toggleDropdown} on:keydown={(e) => e.key === 'Enter' && toggleDropdown(e)}>
  ...
  {#if showDropdown}
    <div class="absolute right-0 mt-1 w-full bg-white border rounded shadow-lg z-10">
      {#each actions as action}
        <div role="button" tabindex="0" class="cursor-pointer p-2 hover:bg-gray-200 text-black" on:click={() => handleSend(action.action)} on:keydown={(e) => e.key === 'Enter' && handleSend(action.action)}>
          {action.label}
        </div>
      {/each}
    </div>
